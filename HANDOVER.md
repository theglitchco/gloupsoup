...
- archive posters:
  ...
  `public/poster-vol3-absence.webp`
  `public/poster-vol4-screening.png`
  `public/poster-vol5-unfinished-business.webp`
...
- Latest local state at handoff includes the archive redesign, imported volume metadata, local poster assets, optimized Volume III/V WebP poster additions from Dropbox links in the Google Sheet, director hover tuning, hero/ethos text animations, the v2 trailer swap with H.264 faststart transcode and less aggressive error fallback, the refreshed overview copy from `public/media/overview.rtf`, archive description updates copied from the sheet metadata tab, and a synced archive poster/title refresh from the Google Sheet.
